...
import { ProductError, ProductSkeleton } from "./product-fallback";
import { useProduct } from "@/hooks/shopify/useProduct";
...
export default function ProductInfo({ productHandle }: Props) {
  ...
  if (!product) {
    const msg = `Sorry couldn't find the product you're looking for :(`;
  ...
  return (
    <div className="min-h-screen flex flex-col gap-8 mb-16 mt-px">
      ...
      {/* <TestimonialsContainer className="md:px-16 px-8" /> */}
    </div>
  );
}
